...
function loadWindowState() {
  try {
    ...
    const content = require$$0.readFileSync(filePath, "utf-8");
...
function requireBindings() {
  ...
  (function(module2, exports$1) {
    var fs2 = require$$0, path$1 = path, fileURLToPath = requireFileUriToPath(), join = path$1.join, dirname = path$1.dirname, exists2 = fs2.accessSync && function(path2) {
      try {
      ...
      return true;
    } || fs2.existsSync || path$1.existsSync, defaults = {
...
function requireDatabase() {
  ...
  function Database2(filenameGiven, options) {
    ...
    if (!anonymous && !filename.startsWith("file:") && !fs2.existsSync(path$1.dirname(filename))) {
...
const settings = sqliteTable("settings", {
  ...
  updatedAt: integer("updated_at", { mode: "timestamp" }).$onUpdate(() => /* @__PURE__ */ new Date()).$defaultFn(() => /* @__PURE__ */ new Date())
});
const appState = sqliteTable("app_state", {
...
const productAttributes = sqliteTable("product_attributes", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
...
const smedStudies = sqliteTable("smed_studies", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  // Basic info
  name: text("name").notNull(),
  description: text("description"),
  // Changeover type - either product references OR free text
  fromProductId: text("from_product_id").references(() => products.id, { onDelete: "set null" }),
  toProductId: text("to_product_id").references(() => products.id, { onDelete: "set null" }),
  changeoverType: text("changeover_type"),
  // Free text alternative to product IDs
  // Location
  ...
  statusIdx: index("smed_studies_status_idx").on(table2.status),
  updatedAtIdx: index("smed_studies_updated_at_idx").on(table2.updatedAt)
}));
const smedSteps = sqliteTable("smed_steps", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  studyId: text("study_id").notNull().references(() => smedStudies.id, { onDelete: "cascade" }),
  ...
  studyIdIdx: index("smed_steps_study_id_idx").on(table2.studyId),
  sequenceIdx: index("smed_steps_sequence_idx").on(table2.studyId, table2.sequenceNumber)
}));
const smedImprovements = sqliteTable("smed_improvements", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  studyId: text("study_id").notNull().references(() => smedStudies.id, { onDelete: "cascade" }),
  description: text("description").notNull(),
  ...
  studyIdIdx: index("smed_improvements_study_id_idx").on(table2.studyId),
  statusIdx: index("smed_improvements_status_idx").on(table2.status)
}));
const smedStandards = sqliteTable("smed_standards", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  studyId: text("study_id").notNull().references(() => smedStudies.id, { onDelete: "cascade" }),
  ...
  visualAidsJson: text("visual_aids_json"),
  ...
  studyIdIdx: index("smed_standards_study_id_idx").on(table2.studyId),
  activeIdx: index("smed_standards_active_idx").on(table2.isActive)
}));
const smedChangeoverLogs = sqliteTable("smed_changeover_logs", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  studyId: text("study_id").notNull().references(() => smedStudies.id, { onDelete: "cascade" }),
  standardId: text("standard_id").references(() => smedStandards.id, { onDelete: "set null" }),
  // Execution details
  operator: text("operator"),
  totalSeconds: integer("total_seconds").notNull(),
  // Step-by-step timing (JSON array)
  stepTimingsJson: text("step_timings_json"),
  // [{ stepId, seconds }]
  // Variance analysis
  ...
  studyIdIdx: index("smed_logs_study_id_idx").on(table2.studyId),
  startedAtIdx: index("smed_logs_started_at_idx").on(table2.startedAt)
}));
const optimizationRuns = sqliteTable("optimization_runs", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  // Optional metadata
  name: text("name"),
  ...
  attributesJson: text("attributes_json").notNull(),
  // JSON of AttributeConfig[]
  attributeStatsJson: text("attribute_stats_json").notNull(),
  // JSON of AttributeStat[]
  // Reference to template used (optional)
  templateId: text("template_id").references(() => templates.id, { onDelete: "set null" }),
  // Timestamp
  runAt: integer("run_at", { mode: "timestamp" }).$defaultFn(() => /* @__PURE__ */ new Date()).notNull(),
  createdAt: integer("created_at", { mode: "timestamp" }).$defaultFn(() => /* @__PURE__ */ new Date())
}, (table2) => ({
  runAtIdx: index("optimization_runs_run_at_idx").on(table2.runAt),
  templateIdIdx: index("optimization_runs_template_id_idx").on(table2.templateId)
}));
const changeoverAttributes = sqliteTable("changeover_attributes", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  // Attribute identification
  name: text("name").notNull(),
  // Internal name: "color", "size"
  displayName: text("display_name").notNull(),
  // UI display: "Color", "Size"
  // Configuration
  ...
  nameIdx: index("changeover_attributes_name_idx").on(table2.name),
  activeIdx: index("changeover_attributes_active_idx").on(table2.isActive)
}));
const changeoverMatrix = sqliteTable("changeover_matrix", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  // Reference to attribute
  attributeId: text("attribute_id").notNull().references(() => changeoverAttributes.id, { onDelete: "cascade" }),
  ...
  smedStudyId: text("smed_study_id").references(() => smedStudies.id, { onDelete: "set null" }),
  notes: text("notes"),
  // Timestamps
  createdAt: integer("created_at", { mode: "timestamp" }).$defaultFn(() => /* @__PURE__ */ new Date()),
  updatedAt: integer("updated_at", { mode: "timestamp" }).$onUpdate(() => /* @__PURE__ */ new Date()).$defaultFn(() => /* @__PURE__ */ new Date())
  ...
  attrFromToIdx: index("changeover_matrix_attr_from_to_idx").on(table2.attributeId, table2.fromValue, table2.toValue),
  attributeIdx: index("changeover_matrix_attribute_idx").on(table2.attributeId)
}));
...
function initDatabase() {
  if (db) return db;
  const dbPath = path.join(electron.app.getPath("userData"), "changeoveroptimizer.db");
...
function createHandler(fn, errorMessage) {
...
function createHandlerWithEvent(fn, errorMessage) {
...
const handlers = [];
function registerHandler(channel, fn, errorMessage) {
  handlers.push({
    channel,
    handler: createHandler(fn, errorMessage)
  });
}
function registerHandlerWithEvent(channel, fn, errorMessage) {
  handlers.push({
    channel,
...
async function getAllTemplates() {
  ...
  const rows = db2.select().from(templates).orderBy(desc(templates.updatedAt)).all();
  return rows.map((row) => ({
    id: row.id,
    name: row.name,
    created: row.createdAt ? row.createdAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
    modified: row.updatedAt ? row.updatedAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
    config: {
...
async function saveTemplate(template) {
  ...
  db2.insert(templates).values(values).onConflictDoUpdate({
    target: templates.id,
    set: {
      name: values.name,
...
async function deleteTemplate(id) {
  const db2 = getDatabase();
  db2.delete(templates).where(eq(templates.id, id)).run();
}
function registerSystemHandlers() {
  registerHandler(
    "system:greet",
    ({ name }) => `Hello, ${name}! You've been greeted from Electron!`,
    "Failed to greet"
  );
  registerHandler(
    "system:read_file",
    async ({ path: path2 }) => {
      const buffer = await fs$1.readFile(path2);
  ...
  registerHandler(
    "system:write_file",
    async ({ path: path2, data }) => {
      const buffer = Buffer.from(data);
      await fs$1.writeFile(path2, buffer);
    },
  ...
  registerHandler("templates:list", () => getAllTemplates(), "Failed to list templates");
  registerHandler(
    "templates:save",
...
function getAllChangeoverAttributes() {
  const db2 = getDatabase();
  return db2.select().from(changeoverAttributes).orderBy(changeoverAttributes.sortOrder).all();
}
function getActiveChangeoverAttributes() {
  const db2 = getDatabase();
  return db2.select().from(changeoverAttributes).where(eq(changeoverAttributes.isActive, true)).orderBy(changeoverAttributes.sortOrder).all();
}
function getChangeoverAttributeById(id) {
  const db2 = getDatabase();
  return db2.select().from(changeoverAttributes).where(eq(changeoverAttributes.id, id)).get();
}
function getChangeoverAttributeByName(name) {
  const db2 = getDatabase();
  return db2.select().from(changeoverAttributes).where(eq(changeoverAttributes.name, name)).get();
}
function createChangeoverAttribute(data) {
  const db2 = getDatabase();
  return db2.insert(changeoverAttributes).values(data).returning().get();
}
function updateChangeoverAttribute(id, data) {
  const db2 = getDatabase();
  return db2.update(changeoverAttributes).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(changeoverAttributes.id, id)).returning().get();
}
function upsertChangeoverAttribute(data) {
  getDatabase();
...
function deleteChangeoverAttribute(id) {
  const db2 = getDatabase();
  db2.delete(changeoverAttributes).where(eq(changeoverAttributes.id, id)).run();
}
function getMatrixByAttribute(attributeId) {
  const db2 = getDatabase();
  return db2.select().from(changeoverMatrix).where(eq(changeoverMatrix.attributeId, attributeId)).all();
}
function getMatrixEntry(attributeId, fromValue, toValue) {
  const db2 = getDatabase();
  return db2.select().from(changeoverMatrix).where(and(
    eq(changeoverMatrix.attributeId, attributeId),
    eq(changeoverMatrix.fromValue, fromValue),
    eq(changeoverMatrix.toValue, toValue)
  )).get();
}
function createMatrixEntry(data) {
  const db2 = getDatabase();
  return db2.insert(changeoverMatrix).values(data).returning().get();
}
function updateMatrixEntry(id, data) {
  const db2 = getDatabase();
  return db2.update(changeoverMatrix).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(changeoverMatrix.id, id)).returning().get();
}
function upsertMatrixEntry(data) {
  getDatabase();
...
function deleteMatrixEntry(id) {
  const db2 = getDatabase();
  db2.delete(changeoverMatrix).where(eq(changeoverMatrix.id, id)).run();
}
function batchGetChangeoverTimes(lookups) {
  const db2 = getDatabase();
  const result = /* @__PURE__ */ new Map();
  const attributes = getActiveChangeoverAttributes();
  const attrByName = new Map(attributes.map((a) => [a.name, a]));
  const attrIds = /* @__PURE__ */ new Set();
  for (const lookup of lookups) {
    const attr = attrByName.get(lookup.attributeName);
    if (attr) attrIds.add(attr.id);
  }
  if (attrIds.size === 0) return result;
  const entries = db2.select().from(changeoverMatrix).where(inArray(changeoverMatrix.attributeId, Array.from(attrIds))).all();
  const matrixByAttr = /* @__PURE__ */ new Map();
  for (const entry of entries) {
    if (!matrixByAttr.has(entry.attributeId)) {
      matrixByAttr.set(entry.attributeId, /* @__PURE__ */ new Map());
    }
...
function prefetchMatrixData(attributeNames, valuesByAttribute) {
  const db2 = getDatabase();
  const result = /* @__PURE__ */ new Map();
  const attributes = getActiveChangeoverAttributes().filter((a) => attributeNames.includes(a.name));
  ...
  const entries = db2.select().from(changeoverMatrix).where(inArray(changeoverMatrix.attributeId, attrIds)).all();
  for (const entry of entries) {
...
function importFromSmedStandard(attributeId, fromValue, toValue, timeMinutes, smedStudyId, notes) {
...
function getAllStudies() {
  const db2 = getDatabase();
  return db2.select().from(smedStudies).orderBy(desc(smedStudies.updatedAt)).all();
}
function getStudyById(id) {
  const db2 = getDatabase();
  return db2.select().from(smedStudies).where(eq(smedStudies.id, id)).get();
}
function createStudy(data) {
  const db2 = getDatabase();
  const result = db2.insert(smedStudies).values(data).returning().get();
  return result;
}
function updateStudy(id, data) {
  const db2 = getDatabase();
  return db2.update(smedStudies).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(smedStudies.id, id)).returning().get();
}
function deleteStudy(id) {
  const db2 = getDatabase();
  db2.delete(smedStudies).where(eq(smedStudies.id, id)).run();
}
function getStepsByStudyId(studyId) {
  const db2 = getDatabase();
  return db2.select().from(smedSteps).where(eq(smedSteps.studyId, studyId)).orderBy(smedSteps.sequenceNumber).all();
}
function createStep(data) {
  const db2 = getDatabase();
  const result = db2.insert(smedSteps).values(data).returning().get();
  return result;
}
function updateStep(id, data) {
  const db2 = getDatabase();
  return db2.update(smedSteps).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(smedSteps.id, id)).returning().get();
}
function deleteStep(id) {
  const db2 = getDatabase();
  db2.delete(smedSteps).where(eq(smedSteps.id, id)).run();
}
function getImprovementsByStudyId(studyId) {
  const db2 = getDatabase();
  return db2.select().from(smedImprovements).where(eq(smedImprovements.studyId, studyId)).orderBy(desc(smedImprovements.createdAt)).all();
}
function createImprovement(data) {
  const db2 = getDatabase();
  const result = db2.insert(smedImprovements).values(data).returning().get();
  return result;
}
function updateImprovement(id, data) {
  const db2 = getDatabase();
  return db2.update(smedImprovements).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(smedImprovements.id, id)).returning().get();
}
function getStandardsByStudyId(studyId) {
  const db2 = getDatabase();
  return db2.select().from(smedStandards).where(eq(smedStandards.studyId, studyId)).orderBy(desc(smedStandards.version)).all();
}
function getActiveStandard(studyId) {
  const db2 = getDatabase();
  return db2.select().from(smedStandards).where(and(
...
function createStandard(data) {
  const db2 = getDatabase();
  const result = db2.insert(smedStandards).values(data).returning().get();
  return result;
}
function updateStandard(id, data) {
  const db2 = getDatabase();
  const result = db2.update(smedStandards).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(smedStandards.id, id)).returning().get();
  return result;
}
function publishStandard(standardId, updateOptimizer = true) {
  const db2 = getDatabase();
  const standard = db2.select().from(smedStandards).where(eq(smedStandards.id, standardId)).get();
  if (!standard) throw new Error("Standard not found");
  db2.update(smedStandards).set({ isActive: false }).where(eq(smedStandards.studyId, standard.studyId)).run();
  const result = db2.update(smedStandards).set({
    isActive: true,
    publishedAt: /* @__PURE__ */ new Date(),
    updatedAt: /* @__PURE__ */ new Date()
  }).where(eq(smedStandards.id, standardId)).returning().get();
  if (updateOptimizer) {
    try {
      const study = db2.select().from(smedStudies).where(eq(smedStudies.id, standard.studyId)).get();
      if (study) {
        if (study.fromProductId && study.toProductId) {
          const fromProduct = db2.select().from(products).where(eq(products.id, study.fromProductId)).get();
          const toProduct = db2.select().from(products).where(eq(products.id, study.toProductId)).get();
          if (fromProduct && toProduct && study.changeoverType) {
            ...
            if (attribute) {
              upsertChangeoverAttribute({
                ...
                // Keep existing display name or update?
                defaultMinutes: standard.standardTimeMinutes,
                hierarchyLevel: attribute.hierarchyLevel,
                // Preserve
                parallelGroup: attribute.parallelGroup,
                // Preserve
                isActive: true
              });
            }
...
function deactivateStandard(standardId) {
  const db2 = getDatabase();
  const result = db2.update(smedStandards).set({
    isActive: false,
    updatedAt: /* @__PURE__ */ new Date()
...
function getLogsByStudyId(studyId, limit = 50) {
  const db2 = getDatabase();
  return db2.select().from(smedChangeoverLogs).where(eq(smedChangeoverLogs.studyId, studyId)).orderBy(desc(smedChangeoverLogs.startedAt)).limit(limit).all();
}
function createChangeoverLog(data) {
  const db2 = getDatabase();
  const result = db2.insert(smedChangeoverLogs).values(data).returning().get();
  return result;
}
function getStudyStatistics(studyId) {
  ...
  const internalTime = steps.filter((s) => s.operationType === "internal").reduce((sum, s) => sum + s.durationSeconds, 0);
  const externalTime = steps.filter((s) => s.operationType === "external").reduce((sum, s) => sum + s.durationSeconds, 0);
  const avgLogTime = logs.length > 0 ? logs.reduce((sum, l) => sum + l.totalSeconds, 0) / logs.length : null;
  return {
    totalSteps,
    internalSteps,
    externalSteps,
...
function registerSmedHandlers() {
  registerHandler("smed:get_all_studies", () => getAllStudies(), "Failed to get studies");
  registerHandler("smed:get_study_by_id", ({ id }) => getStudyById(id), "Failed to get study");
  registerHandler("smed:create_study", ({ data }) => createStudy(data), "Failed to create study");
  registerHandler("smed:update_study", ({ id, data }) => updateStudy(id, data), "Failed to update study");
  registerHandler("smed:delete_study", ({ id }) => deleteStudy(id), "Failed to delete study");
  registerHandler("smed:get_steps", ({ studyId }) => getStepsByStudyId(studyId), "Failed to get steps");
  registerHandler("smed:create_step", ({ data }) => createStep(data), "Failed to create step");
  registerHandler("smed:update_step", ({ id, data }) => updateStep(id, data), "Failed to update step");
  registerHandler("smed:delete_step", ({ id }) => deleteStep(id), "Failed to delete step");
  registerHandler("smed:get_improvements", ({ studyId }) => getImprovementsByStudyId(studyId), "Failed to get improvements");
  registerHandler("smed:create_improvement", ({ data }) => createImprovement(data), "Failed to create improvement");
  registerHandler("smed:update_improvement", ({ id, data }) => updateImprovement(id, data), "Failed to update improvement");
  registerHandler("smed:get_standards", ({ studyId }) => getStandardsByStudyId(studyId), "Failed to get standards");
  registerHandler("smed:get_active_standard", ({ studyId }) => getActiveStandard(studyId), "Failed to get active standard");
  registerHandler("smed:create_standard", ({ data }) => createStandard(data), "Failed to create standard");
  registerHandler("smed:update_standard", ({ id, data }) => updateStandard(id, data), "Failed to update standard");
  registerHandler("smed:publish_standard", ({ standardId, updateOptimizer }) => publishStandard(standardId, updateOptimizer), "Failed to publish standard");
  registerHandler("smed:deactivate_standard", ({ standardId }) => deactivateStandard(standardId), "Failed to deactivate standard");
  registerHandler("smed:get_logs", ({ studyId, limit }) => getLogsByStudyId(studyId, limit), "Failed to get logs");
  registerHandler("smed:create_log", ({ data }) => createChangeoverLog(data), "Failed to create log");
  registerHandler("smed:get_statistics", ({ studyId }) => getStudyStatistics(studyId), "Failed to get statistics");
}
function saveOptimizationRun(data) {
  const db2 = getDatabase();
  const result = db2.insert(optimizationRuns).values(data).returning().get();
  return result;
}
function getOptimizationRuns(limit = 50) {
  const db2 = getDatabase();
  return db2.select().from(optimizationRuns).orderBy(desc(optimizationRuns.runAt)).limit(limit).all();
}
function getOptimizationRunById(id) {
  const db2 = getDatabase();
  return db2.select().from(optimizationRuns).where(eq(optimizationRuns.id, id)).get();
}
function deleteOptimizationRun(id) {
  const db2 = getDatabase();
  db2.delete(optimizationRuns).where(eq(optimizationRuns.id, id)).run();
}
function getOptimizationSavingsTrend(months = 12) {
  const db2 = getDatabase();
  const cutoffDate = /* @__PURE__ */ new Date();
  cutoffDate.setMonth(cutoffDate.getMonth() - months);
  const runs = db2.select().from(optimizationRuns).where(gte(optimizationRuns.runAt, cutoffDate)).orderBy(optimizationRuns.runAt).all();
  const monthlyData = /* @__PURE__ */ new Map();
  for (const run of runs) {
    const date = run.runAt instanceof Date ? run.runAt : new Date(run.runAt);
...
function getTopOptimizationRuns(limit = 10) {
  const db2 = getDatabase();
  return db2.select().from(optimizationRuns).orderBy(desc(optimizationRuns.savingsPercent)).limit(limit).all();
}
function getOptimizationOverviewStats() {
  const db2 = getDatabase();
  const runs = db2.select().from(optimizationRuns).all();
  if (runs.length === 0) {
    return {
      totalRuns: 0,
...
function getSmedOverviewStats() {
  const db2 = getDatabase();
  const studies = db2.select().from(smedStudies).all();
...
function getStudyComparisonData() {
  const db2 = getDatabase();
  const studies = db2.select().from(smedStudies).where(ne(smedStudies.status, "archived")).orderBy(desc(smedStudies.updatedAt)).all();
  if (studies.length === 0) {
    return [];
  }
  const studyIds = studies.map((s) => s.id);
  const allSteps = db2.select().from(smedSteps).where(inArray(smedSteps.studyId, studyIds)).all();
  const stepsByStudyId = /* @__PURE__ */ new Map();
...
function getImprovementTrends(months = 12) {
  const db2 = getDatabase();
  const cutoffDate = /* @__PURE__ */ new Date();
  cutoffDate.setMonth(cutoffDate.getMonth() - months);
  const improvements = db2.select().from(smedImprovements).where(gte(smedImprovements.createdAt, cutoffDate)).orderBy(smedImprovements.createdAt).all();
  const monthlyData = /* @__PURE__ */ new Map();
  for (const imp of improvements) {
    const date = imp.createdAt instanceof Date ? imp.createdAt : new Date(imp.createdAt);
    const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`;
    const existing = monthlyData.get(monthKey) || {
      ideasCreated: 0,
      implementedCount: 0,
...
function getImprovementTypeDistribution() {
  const db2 = getDatabase();
  const improvements = db2.select().from(smedImprovements).all();
  const typeData = /* @__PURE__ */ new Map();
  for (const imp of improvements) {
...
function getOperationTypeBreakdown() {
  const db2 = getDatabase();
  const studies = db2.select().from(smedStudies).where(ne(smedStudies.status, "archived")).all();
  if (studies.length === 0) {
    return {
...
function registerAnalyticsHandlers() {
  registerHandler("analytics:save_run", ({ data }) => saveOptimizationRun(data), "Failed to save optimization run");
  registerHandler("analytics:get_runs", ({ limit }) => getOptimizationRuns(limit), "Failed to get optimization runs");
  registerHandler("analytics:get_run_by_id", ({ id }) => getOptimizationRunById(id), "Failed to get optimization run");
  registerHandler("analytics:delete_run", ({ id }) => deleteOptimizationRun(id), "Failed to delete optimization run");
  registerHandler("analytics:get_savings_trend", ({ months }) => getOptimizationSavingsTrend(months), "Failed to get savings trend");
  registerHandler("analytics:get_top_runs", ({ limit }) => getTopOptimizationRuns(limit), "Failed to get top runs");
  ...
  registerHandler("analytics:get_improvement_trends", ({ months }) => getImprovementTrends(months), "Failed to get improvement trends");
...
function registerChangeoverHandlers() {
  ...
  registerHandler("changeovers:get_attribute_by_id", ({ id }) => getChangeoverAttributeById(id), "Failed to get changeover attribute");
  registerHandler("changeovers:get_attribute_by_name", ({ name }) => getChangeoverAttributeByName(name), "Failed to get changeover attribute by name");
  registerHandler("changeovers:upsert_attribute", ({ data }) => upsertChangeoverAttribute(data), "Failed to upsert changeover attribute");
  registerHandler("changeovers:delete_attribute", ({ id }) => deleteChangeoverAttribute(id), "Failed to delete changeover attribute");
  registerHandler("changeovers:get_matrix", ({ attributeId }) => getMatrixByAttribute(attributeId), "Failed to get matrix entries");
  registerHandler("changeovers:upsert_matrix_entry", ({ data }) => upsertMatrixEntry(data), "Failed to upsert matrix entry");
  registerHandler("changeovers:delete_matrix_entry", ({ id }) => deleteMatrixEntry(id), "Failed to delete matrix entry");
  registerHandler(
  ...
  registerHandler(
    ...
    ({ attributeNames, valuesByAttribute }) => {
